...
class PointGridEncoder(nn.Module):
    def __init__(self, params):
        ...
        self.dynamic_k = p.get('dynamic_k', 12)  # 动态近邻数（默认8，与原KNN一致）
        ...
        self.use_edgeconv = p.get('use_edgeconv', True)
        self.grid_feat = define_convs(**p.grid_conv)


    def forward(self, model_input):
        pc_KNN_pos = model_input['pc_KNN_pos']        # (N_pts, K, 3)
        points = model_input['info']['points']        # (N_pts, 3)

        # -------------------------------------------------
        # Step 1: MLP提取KNN局部特征
        # -------------------------------------------------

        init_feat = self.mlp_feat.forward_simple(pc_KNN_pos)   # (N_pts, K, feat_dim)

        # 每个点的中心特征
        center_feat = torch.mean(init_feat, dim=1, keepdim=True)  # (N_pts,1,feat_dim)

        # 用于动态邻域计算
        center_feat_flat = center_feat.squeeze(1)                # (N_pts,feat_dim)
        ...
        if self.use_max_pooling: 
            # 使用原始KNN邻域
            neighbor_feat = init_feat                             # (N_pts,K,feat_dim)

        else:
            # 使用动态特征邻域
            neighbor_feat, _ = self.dynamic_adj(center_feat_flat) # (N_pts,K,feat_dim)

        # -------------------------------------------------
        # Step 3: 特征聚合
        # -------------------------------------------------

        if not self.use_edgeconv:

            # Max Pooling 聚合
            agg_feat = torch.max(neighbor_feat, dim=1, keepdim=True)[0]  # (N_pts,1,feat_dim)

        else:

            # EdgeConv 聚合
            agg_feat = self.edge_conv(center_feat, neighbor_feat)        # (N_pts,1,feat_dim)
...
if __name__ == '__main__':
    ...
    params = DottedDict({
        'max_pooling': False,  # 启用动态KNN
        'use_edgeconv': True, # 关闭EdgeConve
        'grid_size': 8,
