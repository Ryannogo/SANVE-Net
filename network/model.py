...
class EdgeCubeNet(nn.Module):
    ...
    def predict_curve(self, model_input):
        ...
        # 确保 cid 与 topo_feat 在同一设备
        cid = cid.to(topo_feat.device)  # 修复点1
        cube_feat = topo_feat[cid[:,0], cid[:,1], cid[:,2]]
        ...
        # 确保 out_cube 是布尔类型且与 cid 在同一设备
        out_cube = out_cube.to(cid.device)  # 修复点2
        peid = cid[out_cube]
        
        edge_topo = {
            'grid_size': k,
            'cube_idx': peid.detach().cpu().numpy(),  # 最终转回 CPU 并转 NumPy
        }
...
#前向传播的函数
    ...
    def predict_curve(self, model_input, peid):
        ...
        # 获取模型所在的设备（通过参数或缓冲区）
        device = next(self.encoder.parameters()).device  # 通过参数获取设备
        
        # 确保 peid 是张量，并移动到模型设备
        if not isinstance(peid, torch.Tensor):
            peid = torch.tensor(peid, device=device)
        else:
            peid = peid.to(device)
        
        # 前向计算（topo_feat 在模型设备）
        topo_feat = self.encoder(model_input)
        ...
        # 确保 cube_neighbor 在模型设备
        cube_neighbor = self.cube_neighbor.to(device)
        
        # 特征提取
        cube_edge_feat = topo_feat[peid[:, 0], peid[:, 1], peid[:, 2]]
        peid_nn = peid[:, None, :] + cube_neighbor
        peid_nn = peid_nn.reshape(N_e * 3, 3)
        torch.clamp_(peid_nn, 0, k - 1)
        ...
        # 处理输出
        if self.predict_type == 'BCE':
            out_face = (self.sigmoid(out_face) > 0.5).to(device)
            out_face = out_face.squeeze(-1)
        else:
            out_face = (out_face[..., 0] < out_face[..., 1]).to(device)
