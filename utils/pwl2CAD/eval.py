...
def calc_metrics(pred, gt, max_HD=False, threshold=0.1):
    ...
    # 计算点到点的距离
    dist_pred2gt, _ = tree_gt.query(pred)  # 每个预测点到最近真实点的距离
    dist_gt2pred, _ = tree_pred.query(gt)  # 每个真实点到最近预测点的距离
    ...
    # 基于阈值的匹配指标计算
    TP = np.sum(dist_pred2gt < threshold)  # 预测点中匹配成功的数量
    FP = len(dist_pred2gt) - TP            # 预测点中匹配失败的数量
    FN = np.sum(dist_gt2pred >= threshold) # 真实点中未被匹配的数量
    ...
    # # 马修斯相关系数 (MCC) 相关计算（已注释）
    # denominator = np.sqrt((TP + FP) * (TP + FN) * (FP) * (FN))
...
if __name__ == "__main__":
    ...
    result_file = "NerVE-main/EVAL/eval_k4_val.txt"
    input_list = "NerVE-main/NerVE64Dataset/val.txt"
    distance_threshold = 0.01  # 距离阈值，可根据数据特性调整
    ...
    processed_count = 0  # 统计有效处理的文件夹数量
    ...
    for folder in target_folders:
        ...
        # 构建文件路径
        pred_path = os.path.join(pred_folder, "pred_TT_pwl.pkl")
