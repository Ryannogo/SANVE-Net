...
<!-- 【TODO: 在此插入论文整体框架示意图，类似原NerVE的Teaser.png】 -->
...
<!-- 【TODO: 在此插入图1 SANVE-Net整体网络架构图】 -->

> **图1** &emsp; SANVE-Net整体网络架构。输入点云经优化后的特征提取模块处理，增强后的特征映射至体素空间后，经3D CNN编码与多分支解码器预测，生成结构化三维轮廓表示。

### Neural Volumetric Edge Representation

体素化表示将连续三维曲线离散为规则体积网格，每个立方体包含三个属性：
1. **边缘占据率 o**：$o \in \{0, 1\}$，定义立方体是否包含边缘
2. **边方向 e**：$e_i \in \{0, 1\}, i=1,2,3$，表示立方体是否应与其相邻立方体连接（定义左、下、背面三个方向）
...
For a quick and easy try, we provide demo data and pre-trained model checkpoints:

<!-- 【TODO: 替换为实际的下载链接】 -->
- [demo_data_checkpoint.zip](https://drive.google.com/xxx)

Unzip the downloaded file at the root, then get `demo/` (demo data) and `results/` (checkpoints). In the folder `demo/`, there are point cloud files of CAD models from [ABC dataset](https://deep-geometry.github.io/abc-dataset/). Run the following code to obtain the SANVE-Net prediction and corresponding piecewise-linear (PWL) curves:

```bash
python demo_predict_pwl.py
```

If everything goes well, a file named `pred_sanve_pwl_curve.pkl` will be generated in each CAD model folder. Next, convert these PWL curves to parametric curves:

```bash
python utils/pwl2CAD/CAD_curve.py
```

Finally, render the parametric curves and SANVE-Net PWL curves with Blender:

```bash
blender -b --python utils/visualization_blender/batch_render_curve.py
```

Blender scripts for visualizing curves and the NerVE cube grid can be found in `utils/visualization_blender`.
...
- [SANVE64Dataset.zip](https://drive.google.com/xxx)
...
<!-- 【TODO: 在此插入公式(7)-(11)图片，评价指标公式】 -->

## Results

### Quantitative Results

<!-- 【TODO: 在此插入表1 各方法定量比较结果截图或表格图】 -->

| Method | CD(10⁻³) ↓ | BHD(10⁻²) ↓ | Precision(%) ↑ | Recall(%) ↑ | IoU(%) ↑ |
|:---:|:---:|:---:|:---:|:---:|:---:|
| PIE-NET | 7.4 | 13.18 | — | — | — |
| EC-NET | 3.7 | 12.84 | — | — | — |
| Traditional | 19.647 | 44.1 | 26.23 | 4.05 | 3.32 |
| EdgeFormer | 5.869 | 13.322 | 82.39 | 27.46 | 26.5 |
| NerVE | 1.28 | 6.793 | 94.64 | 84.15 | 82.81 |
| **SANVE-Net (Ours)** | **1.153** | **6.518** | **94.85** | **85.34** | **83.99** |

### Qualitative Results

<!-- 【TODO: 在此插入图6 不同方法在ABC数据集上的轮廓提取效果对比图】 -->

> **图6** &emsp; 不同方法在ABC数据集上的轮廓提取效果对比。

### Ablation Study

#### Module Combination Ablation

<!-- 【TODO: 在此插入表2 模块组合消融试验结果截图或表格图】 -->

| Method | CD(10⁻³) | BHD(10⁻²) | Precision(%) | Recall(%) | IoU(%) |
|:---:|:---:|:---:|:---:|:---:|:---:|
| DN + EdgeConv | 0.836 | 5.915 | 94.85 | 85.34 | 83.99 |
| DN only | 2.984 | 12.794 | 56.93 | 37.33 | 31.41 |
| EdgeConv only | 1.159 | 6.210 | 94.54 | 85.23 | 83.82 |
| NerVE (Baseline) | 0.974 | 6.680 | 94.64 | 84.16 | 82.81 |

<!-- 【TODO: 在此插入图7 使用不同模块的消融结果可视化图】 -->

> **图7** &emsp; 使用不同模块的消融结果。

消融结果表明，动态邻域重构与 EdgeConv 具有显著的互补性：
- 单独使用 EdgeConv 时，边界精度（BHD）提升，但整体点分布（CD）略有偏移
- 单独使用动态邻域时，模型性能显著下降（缺乏有效关系建模）
- 二者联合使用后各项指标均达到最优

#### Neighborhood Size Parameter Analysis

<!-- 【TODO: 在此插入表3 邻域规模参数消融结果截图或表格图】 -->

| Dynamic K | CD(10⁻³) | BHD(10⁻²) | Precision(%) | Recall(%) | IoU(%) |
|:---:|:---:|:---:|:---:|:---:|:---:|
| 4 | 0.870 | 6.474 | 94.73 | 84.28 | 83.01 |
| **8** | **0.836** | **5.915** | **94.85** | **85.34** | **83.99** |
| 12 | 1.241 | 7.480 | 94.25 | 83.34 | 81.88 |

<!-- 【TODO: 在此插入图8 K值消融试验可视化结果图】 -->

> **图8** &emsp; K值消融试验可视化结果。

K=8 在邻域信息完整性和结构一致性之间取得最优平衡，被选为默认参数设置。
...
This codebase is built upon [NerVE](https://github.com/xxx/NerVE). We thank the authors for releasing their code.